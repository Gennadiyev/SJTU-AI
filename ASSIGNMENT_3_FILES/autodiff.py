...
class Node(object):
    ...
    def __rsub__(self, other):
        if isinstance(other, Node):
            ...
            new_node = sub_constby_op(self, other)
        return new_node
    ...
    def __rdiv__(self, other):
        if isinstance(other, Node):
            ...
            new_node = div_constby_op(self, other)
        return new_node
...
class SubConstByOp(Op):
    ...
    def compute(sef, node, input_vals):
        assert len(input_vals) == 1
...
# Please finish MulOp, MulByConstOp
class MulOp(Op):
...
#Please finish DivOp, DivByConstOp, DivConstByOp
class DivOp(Op):
    def __call__(self, node_A, node_B):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A, node_B]
...
#Advanced Part: Please Finish MatMulOp(Matrix Multiply)
class MatMulOp(Op):
...
sub_constby_op = SubConstByOp()
mul_byconst_op = MulByConstOp()
div_byconst_op = DivByConstOp()
div_constby_op = DivConstByOp()
matmul_op = MatMulOp()
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        if (node.inputs):
            output_grad = node_to_output_grad[node]
            input_grad = node.op.gradient(node, output_grad)
            for index, item in enumerate(node.inputs):
                if item in node_to_output_grad:
                    node_to_output_grad[item] += input_grad[index]
                else:
                    node_to_output_grad[item] = input_grad[index]
