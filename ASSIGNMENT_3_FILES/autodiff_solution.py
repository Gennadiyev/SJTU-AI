...
class Node(object):
    ...
    def __rtruediv__(self, other):
        if isinstance(other, Node):
            ...
            new_node = div_const_node_op(other, self)
        return new_node
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if not isinstance(node.op, PlaceholderOp):
                inputs = [node_to_val_map[x] for x in node.inputs]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        inp_gradient = sum_node_list(node_to_output_grads_list[node])
        if not isinstance(node.op, PlaceholderOp):
            node_gradient = node.op.gradient(node, inp_gradient)
        else:
            node_gradient = inp_gradient
        for idx, inp in enumerate(node.inputs):
            if inp not in node_to_output_grads_list:
                node_to_output_grads_list[inp] = []
            node_to_output_grads_list[inp].append(node_gradient[idx])
        node_to_output_grad[node] = node_gradient
    """TODO: Your code here"""
